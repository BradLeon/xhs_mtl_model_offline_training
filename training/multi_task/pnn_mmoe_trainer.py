#!/usr/bin/env python3
"""
PNN-MMOE混合架构多任务学习训练器

基于统一基础架构的轻量级PNN-MMOE训练器实现。

PNN-MMOE模型特点：
- 结合PNN的特征交互能力和MMOE的多任务学习优势
- PNN Product Layer：对embedding进行内积/外积特征交互，增强特征表示
- MMOE架构：多专家网络自动学习任务间的特征共享与独立性
- 特征增强：将原始特征与product特征拼接，形成增强的输入
...
----------
# 1. 基础训练
python training/multi_task/pnn_mmoe_trainer.py \
...
class PNNMMOETrainer(BaseMTLTrainer):
    """PNN-MMOE混合架构多任务学习训练器
    
    继承BaseMTLTrainer，专注于PNN-MMOE模型特定的配置和创建逻辑。
    ...
    def create_model(self, feature_columns: List) -> nn.Module:
        ...
        Returns:
            初始化的PNN-MMOE模型
        """
        logger.info("Creating PNN-MMOE hybrid architecture model...")
        ...
        model = self.model_factory.create_multi_task_model(
            ...
            # PNN特定参数
            use_inner_product=self.config.use_inner_product,
            use_outter_product=self.config.use_outter_product,
            # MMOE特定参数
        ...
        logger.info(f"✅ Created PNN-MMOE hybrid model with:")
        logger.info(f"   PNN settings: inner_product={self.config.use_inner_product}, "
                   f"outter_product={self.config.use_outter_product}")
        logger.info(f"   MMOE settings: num_experts={self.config.num_experts}")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='PNN-MMOE混合架构多任务学习CTR模型训练')
    
    # ============ 基础参数 ============
    parser.add_argument('--input-path', '-i', required=True,
                        help='输入数据路径（文件或目录）')
    parser.add_argument('--output-path', '-o', default="models/pnn_mmoe_training",
                        help='输出目录')
    ...
    # ============ PNN参数 ============
    parser.add_argument('--use-inner-product', action='store_true', default=True,
                        help='使用PNN内积特征交互')
    parser.add_argument('--use-outter-product', action='store_true',
                        help='使用PNN外积特征交互')
    parser.add_argument('--no-inner-product', action='store_true',
                        help='禁用PNN内积特征交互')
    ...
    logger.info("🎉 PNN-MMOE training completed successfully!")
