#!/usr/bin/env python3
"""
PLE (Progressive Layered Extraction) 多任务学习训练器

基于统一基础架构的轻量级PLE训练器实现。

PLE模型特点：
- 渐进式分层提取架构
- 共享专家和任务特定专家的组合
- 多层门控机制实现任务间知识共享与独立性平衡

使用示例：
----------
# 1. 基础训练
python training/multi_task/ple_trainer.py \
...
from training.base.base_config import MultiTaskConfig, create_config_from_args
...
class PLETrainer(BaseMTLTrainer):
    """PLE多任务学习训练器
    
    继承BaseMTLTrainer，专注于PLE模型特定的配置和创建逻辑。
    ...
    def create_model(self, feature_columns: List) -> nn.Module:
        ...
        Returns:
            初始化的PLE模型
        """
        logger.info("Creating PLE (Progressive Layered Extraction) model...")
        ...
        model = self.model_factory.create_multi_task_model(
            ...
            # PLE特定参数
            shared_expert_num=self.config.shared_expert_num,
            specific_expert_num=self.config.specific_expert_num,
        ...
        logger.info(f"✅ Created PLE model with:")
        logger.info(f"   Shared experts: {self.config.shared_expert_num}")
        logger.info(f"   Specific experts: {self.config.specific_expert_num}")
        logger.info(f"   Levels: {self.config.num_levels}")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='PLE多任务学习CTR模型训练')
    
    # ============ 基础参数 ============
    parser.add_argument('--input-path', '-i', required=True,
                        help='输入数据路径（文件或目录）')
    parser.add_argument('--output-path', '-o', default="models/ple_training",
                        help='输出目录')
    ...
    # ============ PLE模型参数 ============
    parser.add_argument('--shared-expert-num', type=int, default=2,
                        help='共享专家数量')
    parser.add_argument('--specific-expert-num', type=int, default=2,
                        help='任务特定专家数量')
    ...
    logger.info("🎉 PLE training completed successfully!")
